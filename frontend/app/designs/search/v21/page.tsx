...
import { Search, ArrowRight, X, Info } from 'lucide-react';
import { mockArticles, mockTrendingKeywords } from '../mockData';

export default function SearchV21() {
    ...
    <div className="w-full min-h-screen bg-[#FCFAF7] text-[#16131D] font-serif p-6 md:p-12 flex items-center justify-center">
      <div className="bg-[#FCFAF7] border-4 border-[#16131D] w-full max-w-6xl max-h-[85vh] overflow-y-auto shadow-[8px_8px_0px_#16131D] flex flex-col p-6 md:p-10 relative">
        ...
        <div className="flex justify-between items-baseline border-b border-[#16131D]/20 pb-4 mb-6">
          <div>
            <span className="font-sans text-[10px] font-bold uppercase tracking-[0.2em] text-[#6E6A7A]">OP.10 / SPLIT BLOCK</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          {/* Middle Column: Split Blocks (Span 2) */}
          <div className="md:col-span-2 space-y-4">
            ...
            <div className="space-y-6 max-h-[450px] overflow-y-auto pr-2 no-scrollbar">
              {filtered.map(art => (
                <div key={art.id} className="border border-[#16131D] bg-[#FCFAF7] flex group hover:shadow-[4px_4px_0px_#16131D] transition-all duration-200">
                  {art.imageUrl && (
                    <div className="w-1/2 border-r border-[#16131D] overflow-hidden shrink-0">
                      <img 
                        src={art.imageUrl} 
                        alt="" 
                        className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-300 animate-fade-in"
                      />
                    </div>
                  )}
                  <div className="w-1/2 p-4 flex flex-col justify-between">
                    <div>
                      ...
                      <h4 className="text-sm font-bold leading-tight mt-1 line-clamp-3 group-hover:underline cursor-pointer">
                        {art.title}
                      </h4>
                    </div>
          ...
          <div className="md:border-l border-[#16131D]/20 md:pl-6 space-y-8">
            <div className="bg-[#16131D]/5 border border-[#16131D]/10 p-4">
              <span className="font-sans text-[10px] font-bold uppercase tracking-widest text-[#16131D] block mb-3 flex items-center gap-1.5 border-b border-[#16131D]/20 pb-1">
                <Info className="w-3.5 h-3.5" /> Summary Indexes
              </span>
              <div className="space-y-2 font-sans text-[10px] font-bold uppercase">
                {['Positive', 'Neutral', 'Negative'].map((s) => {
                  const count = sentimentCounts[s] || 0;
                  const pct = totalResults === 0 ? 0 : Math.round((count / totalResults) * 100);
                  return (
                    <div key={s} className="border border-[#16131D]/20 p-2 flex justify-between items-baseline bg-[#FCFAF7]">
                      <span>{s}</span>
                      <span className="text-xs text-[#6F3FF5]">{pct}%</span>
                    </div>
                  );
                })}
              </div>
