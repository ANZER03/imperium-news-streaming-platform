...
export default function SearchV18() {
    ...
    <div className="w-full min-h-screen bg-[#FCFAF7] text-[#16131D] font-serif p-6 md:p-12 flex items-center justify-center">
      <div className="bg-[#FCFAF7] border-4 border-[#16131D] w-full max-w-6xl max-h-[85vh] overflow-y-auto shadow-[8px_8px_0px_#16131D] flex flex-col p-6 md:p-10 relative">
        ...
        <div className="flex justify-between items-baseline border-b border-[#16131D]/20 pb-4 mb-6">
          <div>
            <span className="font-sans text-[10px] font-bold uppercase tracking-[0.2em] text-[#6E6A7A]">OP.07 / ASYMMETRIC DECK</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          {/* Middle Column: Asymmetric Cards (Span 2) */}
          <div className="md:col-span-2 space-y-6">
            ...
            <div className="space-y-6 max-h-[450px] overflow-y-auto pr-4 pl-2 py-2 no-scrollbar">
              {filtered.map(art => (
                <div key={art.id} className="relative border-2 border-[#16131D] bg-[#FCFAF7] p-4 flex gap-4 shadow-[4px_4px_0px_#16131D] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_#16131D] transition-all duration-200 group">
                  {art.imageUrl && (
                    <div className="w-24 h-24 bg-[#16131D]/5 border border-[#16131D] overflow-hidden shrink-0 -mt-8 -ml-8 shadow-[2px_2px_0px_#16131D] group-hover:-translate-y-1 transition-all">
                      <img 
                        src={art.imageUrl} 
                        alt="" 
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <h4 className="text-base font-bold leading-tight mt-1 group-hover:text-[#6F3FF5] transition-colors cursor-pointer">
                      {art.title}
                    </h4>
                    <p className="text-xs text-[#6E6A7A] font-sans mt-2 line-clamp-2">{art.excerpt}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="md:border-l border-[#16131D]/20 md:pl-6 space-y-8">
            <div className="bg-[#16131D]/5 border border-[#16131D]/10 p-4">
              <span className="font-sans text-[10px] font-bold uppercase tracking-widest text-[#16131D] block mb-3 flex items-center gap-1.5 border-b border-[#16131D]/20 pb-1">
                <PieChart className="w-3.5 h-3.5" /> Radial Breakdown
              </span>
              <div className="flex flex-wrap gap-4 justify-around font-sans text-[9px] font-bold uppercase">
                    ...
                    <div key={s} className="flex flex-col items-center gap-1.5">
                      <div className="relative w-10 h-10 flex items-center justify-center">
                        <svg className="w-full h-full -rotate-90">
                          <circle cx="20" cy="20" r="16" fill="transparent" stroke="#16131D" strokeWidth="1" className="opacity-10" />
                          <circle 
                            cx="20" 
                            cy="20" 
                            r="16" 
                        ...
                        <span className="absolute text-[8px] font-bold text-[#6F3FF5]">{pct}%</span>
                      </div>
                      <span className="text-[7px] text-[#6E6A7A]">{s.substring(0,3)}</span>
                    </div>
                  );
                })}
              </div>
