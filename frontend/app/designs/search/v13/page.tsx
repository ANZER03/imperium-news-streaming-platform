...
export default function SearchV13() {
    ...
    <div className="w-full min-h-screen bg-[#FCFAF7] text-[#16131D] font-serif p-6 md:p-12 flex items-center justify-center">
      <div className="bg-[#FCFAF7] border-4 border-[#16131D] w-full max-w-6xl max-h-[85vh] overflow-y-auto shadow-[8px_8px_0px_#16131D] flex flex-col p-6 md:p-10 relative">
        ...
        <div className="flex justify-between items-baseline border-b border-[#16131D]/20 pb-4 mb-6">
          <div>
            <span className="font-sans text-[10px] font-bold uppercase tracking-[0.2em] text-[#6E6A7A]">OP.02 / TOP IMAGE GRID</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          {/* Middle Column: 2-Column Grid (Span 2) */}
          <div className="md:col-span-2 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-[450px] overflow-y-auto pr-2 no-scrollbar">
              {filtered.map(art => (
                <div key={art.id} className="border border-[#16131D] bg-[#FCFAF7] flex flex-col group hover:shadow-[4px_4px_0px_#16131D] transition-all duration-200">
                  {art.imageUrl && (
                    <div className="h-32 w-full border-b border-[#16131D] overflow-hidden">
                      <img 
                        src={art.imageUrl} 
                        alt={art.title} 
                  ...
                  <div className="p-3 flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      <h4 className="text-sm font-bold leading-tight line-clamp-2 group-hover:underline cursor-pointer">
                        {art.title}
                      </h4>
                    </div>
                    <p className="text-[11px] text-[#6E6A7A] font-sans mt-2 line-clamp-2">{art.excerpt}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="md:border-l border-[#16131D]/20 md:pl-6 space-y-8">
            <div className="bg-[#16131D]/5 border border-[#16131D]/10 p-4">
              <span className="font-sans text-[10px] font-bold uppercase tracking-widest text-[#16131D] block mb-3 flex items-center gap-1.5 border-b border-[#16131D]/20 pb-1">
                <BarChart className="w-3.5 h-3.5" /> Concentric Sentiment
              </span>
              <div className="flex flex-col gap-4 font-sans text-[10px] font-bold uppercase">
                {['Positive', 'Neutral', 'Negative'].map((s, idx) => {
                  const count = sentimentCounts[s] || 0;
                  const pct = totalResults === 0 ? 0 : Math.round((count / totalResults) * 100);
                  const circleSize = 64 - idx * 16;
                  const strokeDash = Math.round(2 * Math.PI * (circleSize / 2 - 2));
                  const strokeOffset = strokeDash - (strokeDash * pct) / 100;
                  return (
                    <div key={s} className="flex items-center gap-3">
                      <svg className="w-10 h-10 -rotate-90 shrink-0">
                        <circle cx="20" cy="20" r={circleSize / 4} fill="transparent" stroke="#16131D" strokeWidth="1.5" className="opacity-10" />
                        <circle 
                          cx="20" 
                          cy="20" 
                          r={circleSize / 4} 
