...
import { Search, ArrowRight, X, Table } from 'lucide-react';
import { mockArticles, mockTrendingKeywords } from '../mockData';

export default function SearchV15() {
    ...
    <div className="w-full min-h-screen bg-[#FCFAF7] text-[#16131D] font-serif p-6 md:p-12 flex items-center justify-center">
      <div className="bg-[#FCFAF7] border-4 border-[#16131D] w-full max-w-6xl max-h-[85vh] overflow-y-auto shadow-[8px_8px_0px_#16131D] flex flex-col p-6 md:p-10 relative">
        ...
        <div className="flex justify-between items-baseline border-b border-[#16131D]/20 pb-4 mb-6">
          <div>
            <span className="font-sans text-[10px] font-bold uppercase tracking-[0.2em] text-[#6E6A7A]">OP.04 / MAGAZINE SPREAD</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          {/* Middle Column: Alternating Hero/Thumbnails (Span 2) */}
            ...
            <div className="space-y-4 max-h-[450px] overflow-y-auto pr-2 no-scrollbar">
              {filtered.map((art, idx) => {
                if (idx === 0) {
                  ...
                  return (
                    <div key={art.id} className="border border-[#16131D] bg-[#FCFAF7] p-4 group hover:shadow-[4px_4px_0px_#16131D] transition-all duration-200">
                      {art.imageUrl && (
                        <div className="h-48 w-full border border-[#16131D]/20 mb-3 overflow-hidden">
                          <img 
                            src={art.imageUrl} 
                            alt={art.title} 
                      ...
                      <div className="flex justify-between items-baseline mb-1">
                        ...
                        <span className="text-[10px] font-sans text-[#6E6A7A]">{art.publishedAt.toLocaleDateString()}</span>
                      </div>
                      <h4 className="text-xl font-bold leading-tight mt-1 group-hover:underline cursor-pointer">
                        {art.title}
                      </h4>
                      <p className="text-sm text-[#6E6A7A] font-sans mt-2 line-clamp-3">{art.excerpt}</p>
                    </div>
                  );
                } else {
                  // Compact cards for subsequent items
                  return (
                    <div key={art.id} className="border-b border-[#16131D]/10 pb-4 last:border-0 group flex gap-4 items-center">
                      {art.imageUrl && (
                        <div className="w-16 h-16 bg-[#16131D]/5 border border-[#16131D]/15 overflow-hidden shrink-0">
                          <img 
                            src={art.imageUrl} 
                            alt={art.title} 
                            className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-300"
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-baseline">
                          <span className="text-[9px] font-sans font-bold text-[#6F3FF5] uppercase tracking-wider">{art.topic}</span>
                          <span className="text-[9px] font-sans text-[#6E6A7A]">{art.publishedAt.toLocaleDateString()}</span>
                        </div>
                        <h4 className="text-sm font-bold leading-tight mt-0.5 line-clamp-1 group-hover:underline cursor-pointer">
                          {art.title}
                        </h4>
                        <p className="text-xs text-[#6E6A7A] font-sans line-clamp-1">{art.excerpt}</p>
                      </div>
                    </div>
                  );
                }
          ...
          <div className="md:border-l border-[#16131D]/20 md:pl-6 space-y-8">
            <div className="bg-[#16131D]/5 border border-[#16131D]/10 p-4">
              ...
              <table className="w-full font-sans text-[10px] text-left border-collapse">
                <thead>
                  <tr className="border-b border-[#16131D]/30 text-[#6E6A7A]">
                    <th className="pb-1 uppercase font-bold">Vector</th>
                    <th className="pb-1 uppercase font-bold text-right">Ratio</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#16131D]/10">
                  {['Positive', 'Neutral', 'Negative'].map((s) => {
                    const count = sentimentCounts[s] || 0;
                    const pct = totalResults === 0 ? 0 : Math.round((count / totalResults) * 100);
                    return (
                      <tr key={s} className="hover:bg-[#16131D]/5">
                        <td className="py-2 font-bold uppercase">{s}</td>
                        <td className="py-2 text-right font-mono text-[#6F3FF5]">{pct}%</td>
                      </tr>
                    );
                  })}
                </tbody>
