...
import { Search, ArrowRight, X, Activity } from 'lucide-react';
import { mockArticles, mockTrendingKeywords } from '../mockData';

export default function SearchV14() {
    ...
    <div className="w-full min-h-screen bg-[#FCFAF7] text-[#16131D] font-serif p-6 md:p-12 flex items-center justify-center">
      <div className="bg-[#FCFAF7] border-4 border-[#16131D] w-full max-w-6xl max-h-[85vh] overflow-y-auto shadow-[8px_8px_0px_#16131D] flex flex-col p-6 md:p-10 relative">
        ...
        <div className="flex justify-between items-baseline border-b border-[#16131D]/20 pb-4 mb-6">
          <div>
            <span className="font-sans text-[10px] font-bold uppercase tracking-[0.2em] text-[#6E6A7A]">OP.03 / TEXT BACKDROP OVERLAY</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          {/* Middle Column: Results List with Backdrops (Span 2) */}
          <div className="md:col-span-2 space-y-4">
            ...
            <div className="space-y-4 max-h-[450px] overflow-y-auto pr-2 no-scrollbar">
              {filtered.map(art => (
                <div key={art.id} className="relative border border-[#16131D] bg-[#FCFAF7] p-5 overflow-hidden group hover:border-[#6F3FF5] transition-colors duration-200">
                  {art.imageUrl && (
                    <div className="absolute inset-0 opacity-[0.08] group-hover:opacity-[0.15] transition-opacity duration-300 pointer-events-none">
                      <img 
                        src={art.imageUrl} 
                        alt="" 
          ...
          <div className="md:border-l border-[#16131D]/20 md:pl-6 space-y-8">
            <div className="bg-[#16131D]/5 border border-[#16131D]/10 p-4">
              <span className="font-sans text-[10px] font-bold uppercase tracking-widest text-[#16131D] block mb-3 flex items-center gap-1.5 border-b border-[#16131D]/20 pb-1">
                <Activity className="w-3.5 h-3.5" /> Sentiment Index
              </span>
              <div className="grid grid-cols-3 gap-2 text-center font-sans text-[10px] font-bold uppercase">
                {['Positive', 'Neutral', 'Negative'].map((s) => {
                  const count = sentimentCounts[s] || 0;
                  const pct = totalResults === 0 ? 0 : Math.round((count / totalResults) * 100);
                  return (
                    <div key={s} className="border border-[#16131D]/20 p-2 bg-[#FCFAF7]">
                      <span className="text-xs text-[#6F3FF5] block mb-1">{pct}%</span>
                      <span className="text-[8px] text-[#6E6A7A] block">{s.substring(0,3)}</span>
                    </div>
                  );
                })}
              </div>
