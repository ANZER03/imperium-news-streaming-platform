...
import { Search, ArrowRight, X, Gauge } from 'lucide-react';
import { mockArticles, mockTrendingKeywords } from '../mockData';

export default function SearchV16() {
    ...
    <div className="w-full min-h-screen bg-[#FCFAF7] text-[#16131D] font-serif p-6 md:p-12 flex items-center justify-center">
      <div className="bg-[#FCFAF7] border-4 border-[#16131D] w-full max-w-6xl max-h-[85vh] overflow-y-auto shadow-[8px_8px_0px_#16131D] flex flex-col p-6 md:p-10 relative">
        ...
        <div className="flex justify-between items-baseline border-b border-[#16131D]/20 pb-4 mb-6">
          <div>
            <span className="font-sans text-[10px] font-bold uppercase tracking-[0.2em] text-[#6E6A7A]">OP.05 / TIMELINE GRAPH</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          {/* Middle Column: Timeline layout (Span 2) */}
          <div className="md:col-span-2 space-y-4 relative">
            <span className="font-sans text-[10px] font-bold uppercase tracking-wider text-[#6E6A7A] block border-b border-[#16131D]/10 pb-1">
              Timeline Results ({filtered.length})
            </span>
            
            <div className="relative max-h-[450px] overflow-y-auto pr-2 no-scrollbar pl-8">
              {/* Timeline central line */}
              <div className="absolute left-[23px] top-4 bottom-4 w-0.5 bg-[#16131D]/20 pointer-events-none" />

              <div className="space-y-6">
                {filtered.map(art => (
                  <div key={art.id} className="relative group flex flex-col gap-1">
                    
                    {/* Node Image */}
                    {art.imageUrl && (
                      <div className="absolute -left-[35px] top-1.5 w-12 h-12 rounded-full border-2 border-[#16131D] bg-[#FCFAF7] overflow-hidden z-10 shadow-sm shrink-0">
                        <img 
                          src={art.imageUrl} 
                          alt="" 
                    ...
                    <div className="pl-6">
                      ...
                      <h4 className="text-base font-bold leading-tight mt-0.5 group-hover:underline cursor-pointer">
                        {art.title}
                      </h4>
          ...
          <div className="md:border-l border-[#16131D]/20 md:pl-6 space-y-8">
            <div className="bg-[#16131D]/5 border border-[#16131D]/10 p-4">
              <span className="font-sans text-[10px] font-bold uppercase tracking-widest text-[#16131D] block mb-3 flex items-center gap-1.5 border-b border-[#16131D]/20 pb-1">
                <Gauge className="w-3.5 h-3.5" /> Sentiment Gauge
              </span>
                  ...
                  return (
                    <div key={s} className="flex justify-between items-center py-1">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-[8px] text-[#6E6A7A]">({count})</span>
                      </div>
                    </div>
                  );
                })}
