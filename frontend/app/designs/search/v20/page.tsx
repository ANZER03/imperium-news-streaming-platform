...
import { Search, ArrowRight, X, Grid } from 'lucide-react';
import { mockArticles, mockTrendingKeywords } from '../mockData';

export default function SearchV20() {
    ...
    <div className="w-full min-h-screen bg-[#FCFAF7] text-[#16131D] font-serif p-6 md:p-12 flex items-center justify-center">
      <div className="bg-[#FCFAF7] border-4 border-[#16131D] w-full max-w-6xl max-h-[85vh] overflow-y-auto shadow-[8px_8px_0px_#16131D] flex flex-col p-6 md:p-10 relative">
        ...
        <div className="flex justify-between items-baseline border-b border-[#16131D]/20 pb-4 mb-6">
          <div>
            <span className="font-sans text-[10px] font-bold uppercase tracking-[0.2em] text-[#6E6A7A]">OP.09 / RETRO SHADOW</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          {/* Middle Column: Retro Shadow Cards (Span 2) */}
          <div className="md:col-span-2 space-y-4">
            ...
            <div className="space-y-6 max-h-[450px] overflow-y-auto pr-4 py-2 no-scrollbar">
              {filtered.map(art => (
                <div key={art.id} className="border-2 border-[#16131D] bg-[#FCFAF7] p-4 flex gap-4 shadow-[4px_4px_0px_#16131D] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_#16131D] transition-all duration-200 group">
                  {art.imageUrl && (
                    <div className="w-24 h-24 bg-[#16131D]/5 border border-[#16131D] overflow-hidden shrink-0">
                      <img 
                        src={art.imageUrl} 
                        alt="" 
          ...
          <div className="md:border-l border-[#16131D]/20 md:pl-6 space-y-8">
            <div className="bg-[#FCFAF7] border-2 border-[#16131D] shadow-[4px_4px_0px_#16131D] p-4">
              <span className="font-sans text-[10px] font-bold uppercase tracking-widest text-[#16131D] block mb-3 flex items-center gap-1.5 border-b border-[#16131D]/20 pb-1">
                <Grid className="w-3.5 h-3.5" /> Dot Matrix Sentiment
              </span>
              <div className="space-y-3 font-sans text-[9px] font-bold uppercase">
                    ...
                    <div key={s} className="space-y-1">
                      ...
                      <div className="flex gap-1">
                        {[...Array(10)].map((_, i) => (
                          <div 
                            key={i} 
                            className={`w-2 h-2 rounded-full border border-[#16131D]/40 ${
                              i < dots ? 'bg-[#6F3FF5]' : 'bg-[#16131D]/10'
                            }`}
                          />
                        ))}
                      </div>
