...
if [[ "${metadata_signal_before}" -eq 0 ]] && [[ "${metadata_data_offset}" -eq 0 ]]; then
  ...
  printf 'Metadata signal topic has offset %s, data topic has offset %s. Skipping snapshot emit.\n' "${metadata_signal_before}" "${metadata_data_offset}"
...
if [[ "${news_signal_before}" -eq 0 ]] && [[ "${news_data_offset}" -eq 0 ]]; then
  ...
  printf 'News signal topic has offset %s, data topic has offset %s. Skipping snapshot emit.\n' "${news_signal_before}" "${news_data_offset}"
...
echo "CDC connectors registered (reference → metadata → news) and signals emitted exactly once."
