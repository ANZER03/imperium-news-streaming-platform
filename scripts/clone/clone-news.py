...
Usage:
    python scripts/clone/clone-news.py --from-id <id> --size <rows>
...
The last imported id is saved to:
    scripts/clone/.state/clone-news-last-id.json
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("Importing into local source-db (APPEND, skip conflicts) ...")
    context.import_csv_chunk("table_news", tmp_file)

    last_id = int(context.local_scalar("SELECT COALESCE(MAX(id), 0) FROM public.table_news") or "0")
    save_last_id(last_id)

    print(f"Done. last_id={last_id}")
    return 0
