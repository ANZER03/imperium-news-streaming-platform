...
class CloneContext:
    ...
    def from_project_root(cls, root_dir: Path) -> "CloneContext":
        ...
        config = CloneConfig(
            root_dir=root_dir,
            state_dir=root_dir / "scripts" / "clone" / ".state",
            tmp_dir=root_dir / "scripts" / "clone" / ".tmp",
            export_dir=root_dir / "scripts" / "clone" / "exports",
    ...
    def import_csv_chunk(self, table_name: str, csv_file: Path) -> None:
        ...
        cols = self.run_local_psql("-At", "-c", cols_sql, capture_output=True).stdout.strip()
        non_id_cols = ", ".join(
            f"{c}=EXCLUDED.{c}" for c in cols.split(", ") if c != "id"
        )
        self.run_local_psql(
            "-c",
            (
                f"INSERT INTO public.{table_name} SELECT * FROM public.{staging_table} "
                f"ON CONFLICT (id) DO UPDATE SET {non_id_cols}"
            ),
        )
